...
while i > 0:
	...
	if playing == True:
		...
		i -= 1
		if i == 1:
			os.system('/usr/bin/xbmc-send -a "Notification(GPIO, Idle Timer OFF in '+ str(interval) +' seconds, 10000)" &> /dev/null')
			time.sleep(interval - 10)
			os.system('/usr/bin/xbmc-send -a "Notification(GPIO, Idle Timer OFF in 10 seconds, 10000)" &> /dev/null')
			time.sleep(10)
